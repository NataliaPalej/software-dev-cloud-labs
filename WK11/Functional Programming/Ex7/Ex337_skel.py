...
# This counts if ALL items in the list of list are single digits
def hCount(f, list):
    if len(list) == 0:
        return 0
    else:
        first = list.pop(0)
        if f(first.copy()) == True:
...
pMax  = lambda list: (0 if len(list)== 0  else list[0]
                        if list[0] >  pMax(list.copy()[1:]) else  pMax(list[1:]) )
